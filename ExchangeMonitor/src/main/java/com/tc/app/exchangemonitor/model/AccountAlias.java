...
@XmlRootElement
@NamedQueries({@NamedQuery(name = "AccountAlias.findAll", query = "SELECT a FROM AccountAlias a"), @NamedQuery(name = "AccountAlias.findByAcctNum", query = "SELECT a FROM AccountAlias a WHERE a.accountAliasPK.acctNum = :acctNum"), @NamedQuery(name = "AccountAlias.findByAcctAddrNum", query = "SELECT a FROM AccountAlias a WHERE a.accountAliasPK.acctAddrNum = :acctAddrNum"), @NamedQuery(name = "AccountAlias.findByAliasSourceCode", query = "SELECT a FROM AccountAlias a WHERE a.accountAliasPK.aliasSourceCode = :aliasSourceCode"), @NamedQuery(name = "AccountAlias.findByAcctAliasName", query = "SELECT a FROM AccountAlias a WHERE a.acctAliasName = :acctAliasName"), @NamedQuery(name = "AccountAlias.findByTransId", query = "SELECT a FROM AccountAlias a WHERE a.transId = :transId")})
public class AccountAlias implements Serializable
{
	...
	private int transId;
	@JoinColumns({@JoinColumn(name = "acct_num", referencedColumnName = "acct_num", nullable = false, insertable = false, updatable = false), @JoinColumn(name = "acct_addr_num", referencedColumnName = "acct_addr_num", nullable = false, insertable = false, updatable = false)})
	...
	public boolean equals(Object object)
	{
		...
		if(!(object instanceof AccountAlias)){ return false; }
		AccountAlias other = (AccountAlias) object;
		if((this.accountAliasPK == null && other.accountAliasPK != null) || (this.accountAliasPK != null && !this.accountAliasPK.equals(other.accountAliasPK))){ return false; }
		return true;
	}
